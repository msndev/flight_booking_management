...
if (f_logged_in() === true) {
	if (isset($_POST['p_pnr']) === true && isset($_POST['p_fno']) === true) {
		...
		$q3 = "SELECT `e_seats_left` FROM `flights_table` WHERE `f_id`='$uid'";
		$r3 = mysql_query($q3);
		if ($r3) {
			...
			mysql_query("UPDATE `flights_table` SET `e_seats_left`='$e_seats_left+btot' WHERE `fno`='$fno'");
		}
	}
}
echo 'Tickets Cancelled. You will  be redirected to your bookings page in 2 seconds!';
