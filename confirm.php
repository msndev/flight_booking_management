		...
		if(isset($_POST['cardnum'])===true && isset($_POST['cvvnum'])===true) {
			...
			if(isset($_POST['chose_to'])===true) {
				if(isset($_POST['chose_fro'])===true) {
					...
					$class = $_POST['class'];
					if (($class==='Economy' && $counta==='1' && $countc==='0') || ($class==='Economy' && $counta==='1' && $countc==='1')) {
						$aname1 = $_POST['aname1'];
						$asex1 = $_POST['asex1'];
						$aage1 = $_POST['aage1'];
						$ect1 = $_POST['ect1'];
						...
						$totc = $counta + $countc;
						$status = 'Booked';
						$q1 = "SELECT * FROM `flights_table` WHERE `fno`='$to'";
						$r1 = mysql_query($q1);
							...
							if(mysql_num_rows($r10)>0) {
								while($row10 = mysql_fetch_assoc($r10)) {
									...
									if($balance>=$etotr) {
										...
										$q11 = "SELECT `e_seats_left` FROM `flights_table` WHERE `fno`='$to'";
										$r11 = mysql_query($q11);
										if($r11) {
											if(mysql_num_rows($r11)>0) {
												while($row11 = mysql_fetch_assoc($r11)) {
													...
													mysql_query("UPDATE `flights_table` SET `e_seats_left`='$dec_seats11' WHERE `fno`='$to'");
												}				
											} 
										...
										$q12 = "SELECT `e_seats_left` FROM `flights_table` WHERE `fno`='$fro'";
										$r12 = mysql_query($q12);
										if($r12) {
											if(mysql_num_rows($r12)>0) {
												while($row12 = mysql_fetch_assoc($r12)) {
													...
													mysql_query("UPDATE `flights_table` SET `e_seats_left`='$dec_seats12' WHERE `fno`='$fro'");
												}				
											} 
											date_default_timezone_set('Asia/Kolkata');
											$a5 = date('H:i:s');
										...
										echo '<h2>SUCCESS!</h2>';
										echo 'Successfully made a payment<br>';
										echo 'of Rs. ' . $etotr;
										echo '<br><br>Do you wish to proceed to book Hotels or cabs?<br><br><br><br><br>';
										echo "<button type = \"submit\" id = \"yes\" name = \"yes\" class = \"btn success\" onclick =\"location.href = 'hotelbook.php';\">Yes, continue to Book Hotels</button>&nbsp;&nbsp;&nbsp;&nbsp;";
										echo "<button type = \"submit\" id = \"yes\" name = \"yes\" class = \"btn success\" onclick =\"location.href = 'cabbook.php';\">Jump directly to book Cabs nearby</button>&nbsp;&nbsp;&nbsp;&nbsp;";
